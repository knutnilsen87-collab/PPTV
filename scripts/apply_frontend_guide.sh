...
# ProPokerTV — Apply Front-End, Layout & Brand Implementation Guide (v2)
# Safer heredocs; avoids $CONTENT indirection to prevent parsing errors.
# Usage: bash scripts/apply_frontend_guide.sh

TS=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR=".backup_frontend_$TS"
...
# Helper to substitute heredoc body dynamically
set_payload(){ PAYLOAD="$1"; }

# 0) Tailwind config extend
TWCFG="tailwind.config.ts"; [ -f tailwind.config.js ] && TWCFG="tailwind.config.js"
set_payload """/** @type {import('tailwindcss').Config} */
const config = {
  darkMode: ['class'],
...
# 1) Global CSS
set_payload """/* Brand tokens */
:root{ --color-primary:#0B0E13; --color-secondary:#111C2D; --color-accent-gold:#FFD166; --color-deep-gold:#CFAE58; --color-emerald-green:#00ECC3; --color-royal-red:#F4355E; --color-text-primary:#EDEDEF; --color-text-secondary:#A6A7AD; --color-border:#2A2B31; }
...
# 2) Fonts
set_payload """import { Inter, Manrope } from 'next/font/google'
...
# 3) Container & Footer
set_payload """import React from 'react'
...
set_payload """import Container from './Container'
...
# 7) ContentCard
set_payload """import Image from 'next/image'
...
"""
write src/components/ToastProvider.tsx

# 10) Skeletons
set_payload """export function Skeleton({ className='' }:{className?:string}){ return <div className={`animate-pulse bg-white/10 ${className}`} /> }
...
"""
write src/components/ChatDock.tsx

# 12) Layout wiring
set_payload """import type { Metadata } from 'next'
import './globals.css'
...
# 13) Home
set_payload """import HeroSlider from '@/components/HeroSlider'
import { ContentCard } from '@/components/ContentCard'
...
echo "\n✓ Applied v2. Backups in $BACKUP_DIR. Restart dev server."
